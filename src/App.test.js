...
describe("App component", () => {
  test("displays the details of a featured puppy when clicked on Sir Waggington", () => {
  ...
  test("does not change the featured puppy when the same puppy is clicked twice", () => {
    ...
    const repeatedFeaturedPuppyName = screen.getByRole("heading", {
      name: /Sir Waggington/i,
    }).textContent;
    expect(repeatedFeaturedPuppyName).toBe(initialFeaturedPuppyName);
  });

  test("displays the details of a featured puppy when clicked on Miss Furbulous", () => {
    // Render the App component
    render(<App />);
